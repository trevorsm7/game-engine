    ...
    bool testCollision(const ICollider* other) const override
    {
        ...
        for (int y = 0; y < rows; ++y)
        {
            for (int x = 0; x < cols; ++x)
            {
                ...
                const float left = transform.getX() + (x * transform.getW() / cols);
                const float right = transform.getX() + ((x + 1) * transform.getW() / cols);
                const float top = transform.getY() + ((rows - y) * transform.getH() / rows);
                const float bottom = transform.getY() + ((rows - y - 1) * transform.getH() / rows);
